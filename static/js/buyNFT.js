...
async function buyNFT(id) {
    ...
    window.alert("priceinwei=" + priceInWei)

    const marketplace = new web3.eth.Contract(MarketplaceABI, MarketplaceDeployedAddress);
    marketplace.setProvider(window.ethereum);
    window.alert("step 4")

    let purchasedItemID = 'purchasedItemID-input-value-' + tokenID

    document.getElementById(purchasedItemID).value = itemID;

    window.alert("step 5")

    await marketplace.methods.purchaseItem(NFTdeployedAddress, itemID).send({ from: ethereum.selectedAddress, value: priceInWei }).then(function(result) {
        console.log(result);
